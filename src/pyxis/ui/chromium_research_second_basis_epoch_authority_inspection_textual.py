...
class SecondBasisEpochAuthorityInspectionPanel(Static):
    ...
    def __init__(
        ...
        self.authority_projection = SecondBasisEpochAuthorityInspection(
    ...
    def from_in_process_handoff(
        ...
        inspection = inspect_chromium_research_second_basis_epoch_in_process_handoff(
            reentry
        )
        return cls(inspection.launch_provenance, inspection.current_state)
    ...
    def update_current_from_controller(
        ...
        *,
        ...
        """Advance only current state through the shared application-side projection."""

        updated = advance_chromium_research_second_basis_epoch_authority_from_controller(
    ...
    def update_current_from_continuation(
        ...
        *,
        ...
        """Advance typed continuation through the shared ancestry-preserving projection."""
    ...
    def _apply_updated_projection(
        self,
        updated: SecondBasisEpochAuthorityInspection,
    ) -> None:
        if not isinstance(updated, SecondBasisEpochAuthorityInspection):
            raise TypeError("updated must be SecondBasisEpochAuthorityInspection.")
        if updated.launch_provenance is not self.launch_provenance:
            raise ValueError(
                "Authority projection update replaced immutable launch provenance."
            )
...
def _render_inspection(
    ...
    return (
        ...
        "\n"
        f"Authority notice: {second_basis_epoch_authority_notice()}"
    )
