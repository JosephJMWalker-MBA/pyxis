...
_THIRD_BASIS_EPOCH_CUMULATIVE_ROLLOVER = _CumulativeCheckpointRolloverMountSpec(
...
class ThirdBasisEpochResearchSessionShell(ResearchSessionShell):
    ...
    async def _save_third_basis_epoch_continuation_checkpoint(self) -> None:
        ...
        if not destination.value.strip():
            ...
            return
        prior = self.third_basis_epoch_reentry
        try:
            checkpoint = (
                persist_chromium_research_third_basis_epoch_continuation_checkpoint(
                    prior,
...
class ThirdBasisEpochContinuationResearchSessionShell(ResearchSessionShell):
    ...
    async def _mount_research_rollover(
        ...
        def create_checkpoint_controls(current, rollover):
            return ThirdBasisEpochResearchSessionCumulativeCheckpointControls(
                current,
                rollover,
            )
        ...
        await _mount_cumulative_checkpoint_after_rollover(
            ...
            rollover=result,
            spec=_THIRD_BASIS_EPOCH_CUMULATIVE_ROLLOVER,
