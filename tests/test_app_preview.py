...
def _actual_artifact_delta(current_artifacts, proposed_artifacts):
    ...
    removed = tuple(path for path in current_by_path if path not in proposed_by_path)
    added = tuple(path for path in proposed_by_path if path not in current_by_path)
    changed = tuple(
        path
...
def test_remove_preview_predictions_match_real_compiler_and_runtime(
    tmp_path: Path,
) -> None:
    spec = create_workspace_spec(
        "Text Lab",
    ...
    actual_added, actual_changed, actual_removed = _actual_artifact_delta(
        current_artifacts,
        proposed_artifacts,
    )
...
def test_preview_restore_normalize_text_is_in_memory_only(
    tmp_path: Path,
    monkeypatch,
) -> None:
    original = create_workspace_spec(
        "Text Lab",
        "Preview-only restoration proof.",
    )
    inspect_only = original.without_capability("normalize_text")
    before = inspect_only.to_canonical_dict()
    monkeypatch.chdir(tmp_path)

    preview = preview_restore_normalize_text(inspect_only)

    assert inspect_only.to_canonical_dict() == before
    assert preview.current_spec is inspect_only
    assert preview.proposed_spec is not inspect_only
    assert preview.proposed_spec == original
    assert preview.proposed_repository.workspace.capabilities == (
        "inspect_text",
        "normalize_text",
    )
    assert preview.delta.added_capabilities == ("normalize_text",)
    assert preview.delta.removed_capabilities == ()
    assert preview.delta.added_artifact_paths == (
        "generated/capabilities/normalize_text.py",
    )
    assert preview.delta.changed_artifact_paths == (
    ...
    assert preview.delta.removed_artifact_paths == ()
    assert preview.delta.added_runtime_keys == ("normalize_text",)
    assert preview.delta.removed_runtime_keys == ()
    assert not tuple(tmp_path.rglob("*"))


def test_restore_preview_predictions_match_real_compiler_and_runtime(
    tmp_path: Path,
) -> None:
    original = create_workspace_spec(
        "Text Lab",
        "Restore preview prediction validation proof.",
    )
    inspect_only = original.without_capability("normalize_text")
    current_repository = build_repository_ir(inspect_only)
    preview = preview_restore_normalize_text(inspect_only)
    ...
    assert tuple(runtime_result) == preview.proposed_spec.capabilities
    assert "normalize_text" in runtime_result


def test_restore_preview_requires_normalize_text_to_be_absent() -> None:
    spec = create_workspace_spec(
        "Text Lab",
        "Invalid duplicate restore preview proof.",
    )

    with pytest.raises(ValueError, match="already contains"):
        preview_restore_normalize_text(spec)
