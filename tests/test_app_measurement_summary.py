...
def test_summary_bundle(tmp_path):
    ...
    assert (summary.envelope, summary.median, summary.mean, summary.dispersion) == (envelope, median, mean, dispersion)
    assert tuple(x.name for x in fields(summary)) == ("envelope", "median", "mean", "dispersion")
    with pytest.raises(ValueError, match="exact"):
        BuildAndRunMeasurementDescriptiveSummaryEvidence(envelope, replace(median), mean, dispersion)
