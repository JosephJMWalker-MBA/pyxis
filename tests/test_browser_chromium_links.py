...
from pyxis.browser import ChromiumPageTarget, ChromiumReadError, read_chromium_page_links
...
def test_read_chromium_page_links_uses_one_fixed_read_only_runtime_command(monkeypatch) -> None:
    ...
    assert snapshot.links[0].ordinal == 1
    assert snapshot.links[0].href == "https://example.test/first"
    assert snapshot.links[0].text_prefix == "First 😀"
    assert snapshot.links[0].text_character_count == 13
    assert snapshot.links[0].text_truncated is True
    assert snapshot.links[1].href == "mailto:research@example.test"
    assert snapshot.links[1].text_truncated is False
    ...
    assert "link.href" in expression
    assert "link.innerText" in expression
...
def test_read_chromium_page_links_rejects_noncontiguous_ordinals(monkeypatch) -> None:
    websocket = _FakeWebSocket(
        [
            {
                "id": 1,
    ...
    with pytest.raises(ChromiumReadError, match="ordinals were not contiguous"):
        read_chromium_page_links(target)
