...
def test_second_epoch_cli_chains_only_explicit_typed_handoff_into_cumulative_shell(
    tmp_path: Path,
    monkeypatch,
) -> None:
    first_dir = tmp_path / "first"
    first_dir.mkdir(parents=True, exist_ok=True)
    _, _, overlay, _ = _persist_valid_overlay(first_dir, stem="38f-cli-first")
    continuation_dir = tmp_path / "continuation"
    continuation_dir.mkdir(parents=True, exist_ok=True)
    values = _persist_valid_continuation(continuation_dir, stem="38f-cli-cont")
    ...
    def fail_continuation_overlay_reload(*args, **kwargs):
        raise AssertionError("38F in-process handoff must not reload a continuation overlay")

    def fail_continuation_lineage_proof(*args, **kwargs):
        raise AssertionError("38F in-process handoff must not synthesize a 38B path proof")

    monkeypatch.setattr(cli, "_run_second_basis_epoch_research_session_shell", fake_first_shell)
    monkeypatch.setattr(
        cli,
    ...
    assert cli.main(["research-shell", "--second-basis-epoch-overlay", str(overlay)]) == 0
    assert "first_lineage" in observed
    assert observed["handoff"] is handoff


def test_second_epoch_cli_normal_close_does_not_launch_cumulative_mode(
    tmp_path: Path,
    monkeypatch,
) -> None:
    _, _, overlay, _ = _persist_valid_overlay(tmp_path, stem="38f-cli-close")
    ...
    def fail_handoff_shell(reentry) -> None:
        ...
        raise AssertionError("normal close must not implicitly change second-epoch modes")
...
def test_second_epoch_first_shell_runner_returns_exact_typed_handoff_from_textual_app(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    assert cli._run_second_basis_epoch_research_session_shell(lineage) is handoff


def test_second_epoch_first_shell_runner_rejects_untyped_app_return(
    tmp_path: Path,
    monkeypatch,
) -> None:
    import pyxis.ui.second_basis_epoch_authority_inspection_shell as shell_module

    _, earned, overlay, _ = _persist_valid_overlay(tmp_path, stem="38f-invalid")
    lineage = prove_chromium_research_second_basis_epoch_shell_lineage(
        earned,
        overlay_source=overlay,
    )
...
def test_raw_handoff_runner_passes_exact_reentry_to_distinct_factory(
    tmp_path: Path,
    monkeypatch,
) -> None:
    import pyxis.ui.second_basis_epoch_authority_inspection_shell as shell_module

    values = _persist_valid_continuation(tmp_path, stem="38f-raw-runner")
    handoff = values[8].fresh_reentry
    observed: dict[str, object] = {}
    ...
    cli._run_second_basis_epoch_continuation_handoff_research_session_shell(handoff)
    assert observed["reentry"] is handoff
    assert observed["ran"] is True


def test_persisted_continuation_cli_route_still_uses_38b_path_proof_family(
    tmp_path: Path,
    monkeypatch,
) -> None:
    values = _persist_valid_continuation(tmp_path, stem="38f-persisted")
    overlay = values[6]
    observed: dict[str, object] = {}
