...
## Current status — through Milestone 47E / D239
...
- **Three concrete changed-basis product crossings:** the first changed-basis flow reaches exact 35C proof and explicit 44H handoff into the root-backed product; the second reaches fresh 37B proof and explicit 46G handoff into the second-basis-epoch product; the third now reaches an exact third public-33B transition, third public-34A root, first post-third-root public-34B edge, explicit 47D shell-local adoption, and 47E fresh-process verification of that exact adopted session through existing 40A.
- **Third-crossing boundary remains narrow:** 47E proves fresh 40A reconstructability from caller-explicit current locators while leaving the mounted session and immutable launch provenance unchanged. It does not persist a 40B restart overlay, create third-epoch launch provenance, infer a current/latest/head branch, or generalize the concrete lineage into an arbitrary `epoch[n]` model.
...
15. [`docs/MILESTONE_47E.md`](docs/MILESTONE_47E.md) — D239, explicit fresh-process verification of that exact adopted third-basis session through existing 40A; this is the current implemented frontier.

The next distinct boundary is proof-gated durable third-epoch restart configuration through existing 40B. That remains separate from 47E and must not be collapsed into fresh-process verification.
