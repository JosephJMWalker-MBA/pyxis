...
## Current status — through Milestone 51B / D282
...
- **Durable exact passage selection and versioned human reasoning:** one caller-owned 18A paragraph-text range can be persisted independently of interpretation, explicitly relinked, organized in the existing human-owned working set, and durably preserved through explicit `research_working_set.v2`. Matching `research_working_set_note.v2` preserves one overall human rationale; 49F adds explicit revision-v2 durability, 49G adds continuation-v2 durability, 49H makes the next 24A human extension version-aware in memory, and 49I then rejoins the already-general `research_working_set_note_revision_edge.v1` lineage by authorizing continuation v2 as an explicit predecessor family. Working-set/note/revision/continuation v1 contracts remain frozen; no parallel edge-v2 lineage is introduced.
- **Bare saved-passage presentation:** 50A projects a retained `exact_range_selection` working-set member into the normal governed read-only rationale/working-set context without manufacturing note text. `human_note_text=None` means no note exists, while an authored empty string remains a real note. Existing note-bearing members render unchanged; no durable format, direct continuation-v2 sequence-start authority, browser control, citation claim, or semantic promotion is added.
- **Version-aware changed-basis preparation:** 50B lets established 33A preserve or append bare exact-range selections when preparing a changed evidence basis. A newly combined basis containing any `exact_range_selection` uses the already-established `research_working_set.v2` + `research_working_set_note.v2` pair; note-only bases continue using frozen v1 writers. Bare candidate presentation shows explicit note absence rather than `None`. The durable 33B transition remains v1-only and explicitly outside 50B.
- **Version-aware changed-basis transition:** 50C keeps `research_session_working_set_transition.v1` and authorizes its existing content-addressed successor references to name either the exact v1/v1 or exact v2/v2 working-set/note pair. Cross-version pairs fail closed. Transition bytes still contain identities only; 34A remains independently closed to v2-backed transitions.
- **Version-aware changed-basis revision root:** 50D keeps `research_session_working_set_transition_revision_root.v1` and lets public 34A root either exact transition-v1 successor pair already authorized by 33B. Root bytes still contain only the exact transition identity plus revised human wording. Unchanged 34B can bridge a v2-backed root-v1 into ordinary edge-v1 continuation; generic 24C still rejects roots directly.
- **Bare-passage first changed-basis product adoption:** 50E is proof-only: the existing 44A→44E shell carries an exact bare `exact_range_selection` candidate through working-set/note v2, transition-v1, root-v1, edge-v1, and explicit 35A governed-session adoption with no production-code change. The adopted endpoint retains the exact bare member and renders note absence correctly; 35B fresh-process re-entry remains a separate boundary.
- **Typed fresh-process re-entry for bare selections:** 50F adds one application-layer `ChromiumResearchExactRangeSelectionReentryLocator` so public 35B can reconstruct a changed-basis bare selection from explicit capture + selection-sidecar paths through public 16C + 49B. The strict persisted 31B plan-v1 and 35C overlay-v1 member-kind vocabularies remain unchanged and reject serialization/decoding of this new locator.
- **Bare-selection 44F fresh-reentry surface:** 50G teaches the existing first changed-basis 44F Textual form to present a bare `exact_range_selection` without manufactured note text and collect exactly an explicit capture path + selection-sidecar path into the 50F typed locator. Public 35B remains authoritative; the mounted governed session is not replaced and no 35C overlay is written.
- **First-root bare persistence and relaunch:** 50H adds a narrowly versioned first-root restart overlay-v2 only where the bare-selection locator vocabulary must be durable; 50I proves that persisted overlay-v2 can relaunch through the established root-backed product without inventing new authority.
- **Ordinary and cumulative continuation above v2 ancestry:** 50J proves unchanged 35D continuation overlay-v1 remains sufficient above first-root overlay-v2 ancestry; 50K proves unchanged 35E cumulative continuation remains sufficient while retaining the exact first-root overlay-v2 anchor and bare saved passage.
- **Second-basis bare fresh reconstruction:** 50L carries another bare saved passage through existing 44A/46A–46D into a second-root governed session, widens only the 46E Textual locator surface to collect capture + selection-sidecar paths, and proves unchanged public 37A can freshly reconstruct the exact second-basis session. Strict 46F/37B overlay-v1 persistence remains mechanically closed to the bare locator, so no second-basis overlay-v2 is introduced.
- **Second-basis bare durable restart configuration:** 50M adds `research_second_basis_epoch_reentry_locator_overlay.v2` only when the exact 37A typed plan contains a bare `exact_range_selection` locator; note-only second-basis histories remain strict overlay-v1. Existing 46F inherits that typed version selection without replacing mounted state or launch provenance, and persisted-format receipts now report the actual v1/v2 version rather than hardcoding v1. Relaunch from second-basis overlay-v2 remains outside 50M.
- **Second-basis bare persisted relaunch:** 50N is proof-only: the existing `research-shell --second-basis-epoch-overlay` path consumes a 50M overlay-v2 through the public loader, fresh 37A reconstruction, persisted-path second-epoch lineage proof, and the established first-checkpoint product. Both the retained first-root bare passage and the second-basis bare passage survive as real no-note selection records. Existing deterministic `research-inspect` output remains unchanged; no CLI, product, lineage, or inspection version branch is added.
- **First continuation above second-basis bare ancestry:** 50O is proof-only: unchanged public 37C persists exact locator-only `research_second_basis_epoch_continuation_locator_overlay.v1` above a 50M 37B overlay-v2, references that prior overlay rather than copying bare-selection locators, and freshly reconstructs both bare passages through the public 37B loader. The existing first-checkpoint product, persisted continuation CLI relaunch, and deterministic inspection all inherit the nested v2 ancestry without version-specific code; cumulative 37D reuse remains a separate boundary.
- **Cumulative continuation above second-basis bare ancestry:** 50P is proof-only: unchanged public 37D extends a persisted 37C-v1 while preserving the exact direct 37B overlay-v2 fixed anchor, appends only the chosen successor edge, and writes another unchanged 37C-v1. Both bare passages and both roots survive fresh cumulative relaunch; the existing cumulative Textual product, continuation CLI, and deterministic inspection inherit the nested v2 ancestry without version propagation or recursive continuation overlays.
- **Third-basis bare fresh reconstruction:** 50Q widens only the existing 47E Textual locator surface so a third changed-basis bare `exact_range_selection` is presented without manufactured note text and collected as explicit capture + selection-sidecar paths. Existing 44A and 47A–47D carry that candidate unchanged, and unchanged public 40A freshly reconstructs all three bare passages across the full nested ancestry. Strict 47F/public-40B overlay-v1 persistence remains mechanically closed to the bare locator, so third-basis durable versioning is still a separate decision.
- **Third-basis bare durable restart configuration:** 50R adds `research_third_basis_epoch_reentry_locator_overlay.v2` only when the exact public-40A typed plan contains a bare `exact_range_selection` locator. Note-only third-basis histories remain strict overlay-v1, the existing 47F product inherits public-40B version selection without new controls, and persistence receipts report the actual v1/v2 format rather than hardcoding v1. Persisted third-basis overlay-v2 relaunch remains a separate boundary.
- **Third-basis bare persisted relaunch:** 50S is proof-only: the existing `research-shell --third-basis-epoch-overlay` path consumes a real 50R overlay-v2 through the public 40B loader, unchanged public 40A reconstruction, existing 41A persisted-path proof, and the established inspectable first-checkpoint product. All three nested bare passages survive as real no-note selections, and existing deterministic `research-inspect` remains inspection-v1 without any CLI, product, lineage, or inspection version branch.
- **First continuation above third-basis bare ancestry:** 50T is proof-only: unchanged public 40C persists exact locator-only `research_third_basis_epoch_continuation_locator_overlay.v1` above a real 50R 40B overlay-v2, references that prior overlay rather than copying bare-selection locators, and freshly reconstructs all three bare passages through the public 40B loader. The existing first-checkpoint product, persisted continuation CLI relaunch, and deterministic inspection all inherit the v2-backed third-basis ancestry without version-specific code; cumulative 40D reuse remains a separate boundary.
- **Cumulative continuation above third-basis bare ancestry:** 50U is proof-only: unchanged public 40D extends a persisted 40C-v1 while preserving the exact direct 40B overlay-v2 fixed anchor, appends only the chosen successor edge, and writes another unchanged 40C-v1. All three bare passages and all three roots survive fresh cumulative relaunch; the existing cumulative Textual product, continuation CLI, and deterministic inspection inherit the v2-backed ancestry without version propagation or recursive continuation overlays.
- **Third-basis bare same-process handoff:** 50V is proof-only: after a real bare 47F/public-40B overlay-v2 persistence, the existing explicit 47G action transfers exactly `checkpoint.fresh_reentry` into the pathless third-basis first-checkpoint receiver. The saved v2 overlay can disappear before handoff, all three bare passages still survive, launch inspection remains pathless, and the next ordinary rollover mounts the existing blank 40C surface without any handoff/version-specific production change.
- **Bare 47G → 40C → 41E composition:** 50W is proof-only: the exact pathless bare 47G receiver can explicitly checkpoint its first rollover through unchanged public 40C using the researcher-supplied 40B-v2 path, then explicitly transfer `checkpoint.fresh_reentry` through existing 41E even after the new 40C file disappears. The pathless cumulative receiver retains typed 40C authority, mounts the next blank 40D form after rollover, and deliberately does not promote that visible one-hop candidate into inspection current state before a successful 40D proof.
- **Bare 47G → 40C → 41E → 40D cumulative promotion:** 50X is proof-only: the pathless 41E receiver can explicitly supply the current 40C-v1 path and cumulative destinations to unchanged public 40D, which preserves the direct third-basis 40B-v2 anchor, appends only the chosen successor, writes another locator-only 40C-v1, and promotes the exact fresh cumulative typed result. The explicit current-overlay operation path never backfills into immutable pathless 41E launch provenance.
- **CLI exact-passage save from durable evidence:** 50Y adds thin `pyxis research-save-selection` composition over existing public 16C → 17A → 18A → 49A boundaries. The researcher supplies one explicit durable capture, paragraph ordinal, Unicode code-point range, and no-overwrite destination; Pyxis writes the unchanged bare-selection sidecar, freshly verifies it, and emits a deterministic operation receipt without selected text, browser acquisition, source discovery, note/citation semantics, or new durable format.
- **Saved passage into first changed-basis product:** 50Z extends `research-shell --plan` with one exact paired `--candidate-capture` + `--candidate-selection` input. The CLI freshly relinks the saved 49A sidecar through public 49B against the explicit capture, then passes that exact bare loaded record into the established 44A→44H first changed-basis runner. Plain `--plan` launch is unchanged, later lineage entry families remain closed to candidate injection, and no browser/application/UI/durable-format authority is widened.
- **CLI capture from one already-open page:** 51A adds thin `pyxis research-capture` composition over unchanged public 16A → 16B. The researcher supplies a caller-owned DevTools endpoint, an optional exact target id, and a no-overwrite destination; Pyxis performs only the established bounded sequential read-only bundle acquisition, persists the unchanged `research_capture.v1`, freshly verifies it, and emits an operation receipt. Multiple-target ambiguity still requires an exact target id, and the command adds no navigation, active-tab inference, browser mutation, source-authentication, or semantic authority.
- **Interactive exact-passage save from durable evidence:** 51B extends the existing `research-save-selection` action with an optional `--interactive` Textual mode while leaving coordinate-explicit saving intact. The CLI freshly loads one explicit durable capture through public 16C before lazily importing the UI; the researcher chooses only among returned bounded paragraphs and selects text in a read-only `TextArea`. The UI requires rendered text to remain byte-for-byte Python-string equal to the exact loaded paragraph prefix, converts Textual locations to Unicode code-point indexes, delegates to unchanged public 17A → 18A, and returns the exact 18A selection to unchanged public 49A persistence/verification. It adds no live-browser reacquisition, source discovery, fuzzy anchoring, note/citation semantics, or durable format.
...
- **Bounded changed-basis reuse:** after three complete concrete changed-basis product crossings, 48A–48D privately share only triply-proven proof-to-persistence mounting, explicit restart-persistence path submission, post-persistence typed-handoff surface mechanics, and normal-close/validate/receiver-run runner mechanics. Verification, path meaning, controls typing, source construction, result typing, ancestry, persistence, public runners, receiver selection, and root/epoch semantics remain concrete.
...
The detailed narrative below remains the original central proof trail through **Milestone 18C / D137** and is intentionally preserved rather than rewritten wholesale. For current orientation, continue through [`docs/CURRENT_FRONTIER.md`](docs/CURRENT_FRONTIER.md), [`docs/CURRENT_FRONTIER_35_36.md`](docs/CURRENT_FRONTIER_35_36.md), [`docs/CURRENT_FRONTIER_37_38.md`](docs/CURRENT_FRONTIER_37_38.md), [`docs/CURRENT_FRONTIER_39_40.md`](docs/CURRENT_FRONTIER_39_40.md), [`docs/CURRENT_FRONTIER_41_43.md`](docs/CURRENT_FRONTIER_41_43.md), [`docs/CURRENT_FRONTIER_44.md`](docs/CURRENT_FRONTIER_44.md), [`docs/MILESTONE_44H.md`](docs/MILESTONE_44H.md), [`docs/MILESTONE_45A.md`](docs/MILESTONE_45A.md), [`docs/MILESTONE_45B.md`](docs/MILESTONE_45B.md), [`docs/CURRENT_FRONTIER_46.md`](docs/CURRENT_FRONTIER_46.md), [`docs/CURRENT_FRONTIER_49.md`](docs/CURRENT_FRONTIER_49.md), [`docs/MILESTONE_50H.md`](docs/MILESTONE_50H.md), [`docs/MILESTONE_50I.md`](docs/MILESTONE_50I.md), [`docs/MILESTONE_50J.md`](docs/MILESTONE_50J.md), [`docs/MILESTONE_50K.md`](docs/MILESTONE_50K.md), [`docs/MILESTONE_50L.md`](docs/MILESTONE_50L.md), [`docs/MILESTONE_50M.md`](docs/MILESTONE_50M.md), [`docs/MILESTONE_50N.md`](docs/MILESTONE_50N.md), [`docs/MILESTONE_50O.md`](docs/MILESTONE_50O.md), [`docs/MILESTONE_50P.md`](docs/MILESTONE_50P.md), [`docs/MILESTONE_50Q.md`](docs/MILESTONE_50Q.md), [`docs/MILESTONE_50R.md`](docs/MILESTONE_50R.md), [`docs/MILESTONE_50S.md`](docs/MILESTONE_50S.md), [`docs/MILESTONE_50T.md`](docs/MILESTONE_50T.md), [`docs/MILESTONE_50U.md`](docs/MILESTONE_50U.md), [`docs/MILESTONE_50V.md`](docs/MILESTONE_50V.md), [`docs/MILESTONE_50W.md`](docs/MILESTONE_50W.md), [`docs/MILESTONE_50X.md`](docs/MILESTONE_50X.md), [`docs/MILESTONE_50Y.md`](docs/MILESTONE_50Y.md), [`docs/MILESTONE_50Z.md`](docs/MILESTONE_50Z.md), [`docs/MILESTONE_51A.md`](docs/MILESTONE_51A.md), and [`docs/MILESTONE_51B.md`](docs/MILESTONE_51B.md). Milestone-specific records, implementation, tests, and executed CI remain stronger authority than any compact summary.
...
14. [`docs/MILESTONE_49C.md`](docs/MILESTONE_49C.md) — D248, bare relinked exact-range selections become a fourth in-memory 20A working-set member family while durable working-set v1 remains closed.
15. [`docs/MILESTONE_49D.md`](docs/MILESTONE_49D.md) — D249, explicit `research_working_set.v2` persistence and 20C relinking for the expanded member family while the original v1 writer remains frozen.
16. [`docs/MILESTONE_49E.md`](docs/MILESTONE_49E.md) — D250, explicit `research_working_set_note.v2` persistence and 21C relinking for one overall human rationale over a v2 working set while the original note v1 contract remains frozen.
17. [`docs/MILESTONE_49F.md`](docs/MILESTONE_49F.md) — D251, explicit `research_working_set_note_revision.v2` persistence and 22C relinking for one append-only change of a note-v2 rationale while revision v1 remains frozen.
18. [`docs/MILESTONE_49G.md`](docs/MILESTONE_49G.md) — D252, explicit `research_working_set_note_revision_continuation.v2` persistence and 23C relinking for a second append-only human wording change on the v2 line while continuation v1 remains frozen.
19. [`docs/MILESTONE_49H.md`](docs/MILESTONE_49H.md) — D253, existing 24A becomes version-aware for the next in-memory human revision while durable edge authority remains a separate decision.
20. [`docs/MILESTONE_49I.md`](docs/MILESTONE_49I.md) — D254, continuation v2 enters the already-general edge-v1 predecessor seam and ordinary 24C/25A/25B edge behavior resumes without introducing edge v2.
21. [`docs/MILESTONE_50A.md`](docs/MILESTONE_50A.md) — D255, one bare exact-range selection already retained by a valid working set becomes first-class in the governed read-only working-set presentation without inventing a human note or widening durable authority.
22. [`docs/MILESTONE_50B.md`](docs/MILESTONE_50B.md) — D256, established 33A preparation becomes member-vocabulary-aware: bare selections select the existing v2 working-set/note pair while v1-only bases stay v1.
23. [`docs/MILESTONE_50C.md`](docs/MILESTONE_50C.md) — D257, existing transition-v1 accepts either exact supported v1/v1 or v2/v2 successor references and freshly relinks them without a transition-v2 format.
24. [`docs/MILESTONE_50D.md`](docs/MILESTONE_50D.md) — D258, existing root-v1 accepts either exact transition-v1 successor pair through public 34A and the unchanged 34B bridge returns that v2-backed lineage to ordinary edge-v1 continuation without widening generic 24C.
25. [`docs/MILESTONE_50E.md`](docs/MILESTONE_50E.md) — D259, proof-only existing 44A→44E product crossing carries one bare saved passage through v2 preparation, transition-v1, root-v1, edge-v1, and explicit in-process governed-session adoption with no production-code change.
26. [`docs/MILESTONE_50F.md`](docs/MILESTONE_50F.md) — D260, typed 35B fresh-process re-entry can explicitly reload a bare exact-range selection through public 16C + 49B while the strict 31B plan-v1 and 35C overlay-v1 serialization vocabularies remain closed.
27. [`docs/MILESTONE_50G.md`](docs/MILESTONE_50G.md) — D261, the existing 44F product form can explicitly collect capture + selection-sidecar locators for a bare saved passage, faithfully present note absence, and delegate to public 35B without replacing the mounted session or writing restart configuration.
28. [`docs/MILESTONE_50H.md`](docs/MILESTONE_50H.md) — D262, public first-root 35C persistence selects a strict overlay-v2 only when a typed bare-selection locator is present while 31B plan-v1, root-backed overlay-v1, and later-epoch overlay-v1 contracts remain frozen; the unchanged 44G UI inherits that persistence path.
29. [`docs/MILESTONE_50I.md`](docs/MILESTONE_50I.md) — D263, proof-only existing CLI/45A product relaunch consumes that first-root overlay-v2 unchanged: fresh 35B reconstruction retains the bare saved passage without inventing a note, and the existing deterministic read-only inspection path works without another format.
30. [`docs/MILESTONE_50J.md`](docs/MILESTONE_50J.md) — D264, proof-only existing 35D continuation overlay-v1 composes above first-root overlay-v2 ancestry: one ordinary continuation checkpoints and fresh-relaunches without losing or reinterpreting the bare saved passage.
31. [`docs/MILESTONE_50K.md`](docs/MILESTONE_50K.md) — D265, proof-only existing 35E/36C cumulative loop composes above that same direct first-root overlay-v2 anchor: a second ordinary continuation grows the post-root edge tuple while continuation overlay-v1, product relaunch, and read-only inspection remain unchanged. This is the current implemented frontier.

49A–49I provide save → verify → explicit relink → organization → versioned working-set durability → versioned overall human rationale → versioned revision → versioned continuation → one further in-memory revision → re-entry into the existing general edge-v1 lineage. 50A makes that bare-passage state faithfully visible; 50B permits changed-basis preparation without dropping it; 50C carries that basis through the existing content-addressed 33B transition relationship; 50D roots the first post-change revision in root-v1 and rejoins ordinary edge-v1 through unchanged 34B; 50E proves the existing first changed-basis shell can explicitly adopt that lineage as the active governed session; 50F freshly reconstructs that root-backed session from an explicit typed bare-selection locator; 50G exposes that exact 35B proof through the existing 44F Textual workflow; 50H persists that exact proven first-root locator plan as strict root-backed overlay-v2 while preserving all historical v1 configuration contracts; 50I proves that saved v2 plan survives an actual fresh-process CLI relaunch into the established root-backed product and read-only inspection path with no production-code change; 50J proves the unchanged 35D continuation-v1 contract can checkpoint and fresh-relaunch one ordinary continuation above that v2-backed ancestry; 50K proves unchanged 35E/36C cumulative checkpointing can extend that continuation while preserving the same direct first-root v2 anchor. Do not infer second-/third-epoch overlay support for the new locator, a 31B plan-v2, generic 24C root input, browser highlighting, automatic adoption, tags, search, or export from these crossings.
