...
## Current status — through Milestone 50B / D256
...
22. [`docs/MILESTONE_50B.md`](docs/MILESTONE_50B.md) — D256, established 33A preparation becomes member-vocabulary-aware: bare selections select the existing v2 working-set/note pair while v1-only bases stay v1; 33B transition authority remains closed. This is the current implemented frontier.

49A–49I provide save → verify → explicit relink → organization → versioned working-set durability → versioned overall human rationale → versioned revision → versioned continuation → one further in-memory revision → re-entry into the existing general edge-v1 lineage. 50A makes that bare-passage state faithfully visible; 50B then permits the researcher to prepare a changed durable evidence basis without dropping that state or forcing note-only formats. Do not infer 33B transition support, browser highlighting, automatic adoption, tags, search, export, or direct continuation-v2 sequence-start support from these preparation/presentation crossings.
