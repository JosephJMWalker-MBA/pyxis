...
## Current status — through Milestone 49G / D252
...
- **Durable exact passage selection and versioned human reasoning:** one caller-owned 18A paragraph-text range can be persisted independently of interpretation, explicitly relinked, organized in the existing human-owned working set, and durably preserved through explicit `research_working_set.v2`. Matching `research_working_set_note.v2` preserves one overall human rationale; 49F adds explicit `research_working_set_note_revision.v2` persistence/relinking for one append-only human change, and 49G adds explicit `research_working_set_note_revision_continuation.v2` persistence/relinking for the next append-only human change. Original v1 durable contracts remain frozen, and later revision-edge/history products remain intentionally closed until separately earned.
...
18. [`docs/MILESTONE_49G.md`](docs/MILESTONE_49G.md) — D252, explicit `research_working_set_note_revision_continuation.v2` persistence and 23C relinking for a second append-only human wording change on the v2 line while continuation v1 and later edge/history authority remain frozen; this is the current implemented frontier.

49A–49G now provide save → verify → explicit relink → organization → versioned working-set durability → versioned overall human rationale → one versioned append-only human revision → one versioned durable continuation of that revision. The next distinct researcher action, if required, is a further revision beyond the loaded continuation; existing 24A/24B revision-edge machinery remains a separate authority boundary rather than being widened automatically.
