...
13. [`docs/MILESTONE_49A.md`](docs/MILESTONE_49A.md) — D246, durable exact-range selection without a note: canonical file-local persistence of explicit paragraph/range coordinates over durable capture identity.
14. [`docs/MILESTONE_49B.md`](docs/MILESTONE_49B.md) — D247, explicit fail-closed relinking of one freshly verified 49A sidecar to one caller-supplied matching loaded capture through public 17A/18A reconstruction; this is the current implemented frontier.

49A–49B now provide a complete save → verify → explicitly relink passage lifecycle without quote copying, source discovery, fuzzy re-anchoring, or citation/semantic authority. The next milestone must answer another concrete researcher action rather than promoting this exact-range lifecycle into stronger evidentiary claims by default.
