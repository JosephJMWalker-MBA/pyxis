...
## Current status — through Milestone 49I / D254
...
20. [`docs/MILESTONE_49I.md`](docs/MILESTONE_49I.md) — D254, continuation v2 enters the already-general edge-v1 predecessor seam and ordinary 24C/25A/25B edge behavior resumes without introducing edge v2; this is the current implemented frontier.

49A–49I now provide save → verify → explicit relink → organization → versioned working-set durability → versioned overall human rationale → versioned revision → versioned continuation → one further in-memory revision → re-entry into the existing general edge-v1 lineage. That rejoin is a natural plateau: do not infer direct continuation-v2 sequence-declaration support, a new edge format, or broader history authority unless a concrete researcher action requires it.
