...
- **Interactive exact-passage save from durable evidence:** 51B extends the existing `research-save-selection` action with an optional `--interactive` Textual mode while leaving coordinate-explicit saving intact. The CLI freshly loads one explicit durable capture through public 16C before lazily importing the UI; the researcher chooses only among returned bounded paragraphs and selects text in a read-only `TextArea`. The UI requires rendered text to remain exactly equal as a Python string to the loaded paragraph prefix, converts Textual locations to Unicode code-point indexes, delegates to unchanged public 17A → 18A, and returns the exact 18A selection to unchanged public 49A persistence/verification. It adds no live-browser reacquisition, source discovery, fuzzy anchoring, note/citation semantics, or durable format.
