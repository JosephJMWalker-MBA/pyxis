...
## Current status — through Milestone 50C / D257
...
23. [`docs/MILESTONE_50C.md`](docs/MILESTONE_50C.md) — D257, existing transition-v1 accepts either exact supported v1/v1 or v2/v2 successor references and freshly relinks them without a transition-v2 format; 34A remains closed. This is the current implemented frontier.

49A–49I provide save → verify → explicit relink → organization → versioned working-set durability → versioned overall human rationale → versioned revision → versioned continuation → one further in-memory revision → re-entry into the existing general edge-v1 lineage. 50A makes that bare-passage state faithfully visible; 50B permits changed-basis preparation without dropping it; 50C carries that exact prepared basis through the existing content-addressed 33B transition relationship. Do not infer 34A/root support, browser highlighting, automatic adoption, tags, search, export, or direct continuation-v2 sequence-start support from these crossings.
