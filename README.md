...
## Current status — through Milestone 49E / D250
...
- **Durable exact passage selection and pre-interpretive organization:** one caller-owned 18A paragraph-text range can be persisted independently of any note or comparison, explicitly relinked to one supplied matching capture, admitted to the existing human-owned 20A working set before per-selection interpretation, and durably preserved through explicit `research_working_set.v2`. 49E adds matching explicit `research_working_set_note.v2` persistence/relinking for one overall human rationale over that v2 set while freezing the original v1 note contract. Durable revision/history products remain intentionally v1-only.
...
16. [`docs/MILESTONE_49E.md`](docs/MILESTONE_49E.md) — D250, explicit `research_working_set_note.v2` persistence and 21C relinking for one overall human rationale over a v2 working set while the original note v1 contract remains frozen; this is the current implemented frontier.

49A–49E now provide save → verify → explicit relink → in-memory organization → versioned working-set durability → versioned overall human rationale for exact passages before per-selection interpretation. The next distinct boundary is durable revision of a note v2 predecessor; the established revision v1 format remains intentionally closed and must not be widened implicitly.
