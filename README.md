...
## Current status — through Milestone 47F / D240
...
- **Three concrete changed-basis product crossings:** the first changed-basis flow reaches exact 35C proof and explicit 44H handoff into the root-backed product; the second reaches fresh 37B proof and explicit 46G handoff into the second-basis-epoch product; the third now reaches an exact third public-33B transition, third public-34A root, first post-third-root public-34B edge, explicit 47D shell-local adoption, 47E fresh-process verification through existing 40A, and 47F proof-gated persistence of strict 40B restart configuration for that exact historical session.
- **Third-crossing boundary remains narrow:** 47F persists a strict locator-only 40B overlay only after mandatory fresh three-layer proof and leaves the mounted session plus immutable launch provenance unchanged. It does not relaunch or promote from that overlay, create new third-epoch launch provenance, infer a current/latest/head branch, or generalize the concrete lineage into an arbitrary `epoch[n]` model.
...
16. [`docs/MILESTONE_47F.md`](docs/MILESTONE_47F.md) — D240, proof-gated persistence of strict 40B restart configuration for that exact historical third-basis session; this is the current implemented frontier.

The next distinct boundary is explicit typed handoff into the already-established third-basis-epoch product using the exact mandatory fresh proof earned by public 40B. Persistence itself must not be treated as automatic promotion.
