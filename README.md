...
## Current status — through Milestone 49D / D249
...
- **Durable exact passage selection:** one caller-owned 18A paragraph-text range can now be persisted independently of any note or comparison as canonical no-overwrite JSON containing only durable capture identity plus explicit paragraph/range coordinates. 49B explicitly relinks it to one caller-supplied matching loaded capture; 49C admits that bare selection to the existing human-owned 20A working set before per-selection interpretation; and 49D adds an explicit `research_working_set.v2` writer/reader path that durably preserves the expanded member family while freezing the original v1 writer and vocabulary. Durable working-set-note/rationale products remain v1-only.
...
15. [`docs/MILESTONE_49D.md`](docs/MILESTONE_49D.md) — D249, explicit `research_working_set.v2` persistence and 20C relinking for the expanded member family while the original v1 writer remains frozen; this is the current implemented frontier.

49A–49D now provide save → verify → explicit relink → in-memory organization → explicit versioned durability for exact passages before per-selection interpretation. The next distinct boundary is durable human rationale over a v2 working-set parent; 21B remains intentionally v1-only and must not be widened implicitly.
