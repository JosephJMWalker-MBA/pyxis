...
## Current status — through Milestone 49F / D251
...
17. [`docs/MILESTONE_49F.md`](docs/MILESTONE_49F.md) — D251, explicit `research_working_set_note_revision.v2` persistence and 22C relinking for one append-only change of a note-v2 rationale while revision v1 remains frozen; this is the current implemented frontier.

49A–49F now provide save → verify → explicit relink → organization → versioned working-set durability → versioned overall human rationale → one versioned append-only human revision. The next distinct boundary is a second durable revision step on the v2 line; the established continuation v1 format remains intentionally closed.
