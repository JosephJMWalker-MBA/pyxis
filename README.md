...
## Current status — through Milestone 50D / D258
...
24. [`docs/MILESTONE_50D.md`](docs/MILESTONE_50D.md) — D258, existing root-v1 accepts either exact transition-v1 successor pair through public 34A and the unchanged 34B bridge returns that v2-backed lineage to ordinary edge-v1 continuation without widening generic 24C; this is the current implemented frontier.

49A–49I provide save → verify → explicit relink → organization → versioned working-set durability → versioned overall human rationale → versioned revision → versioned continuation → one further in-memory revision → re-entry into the existing general edge-v1 lineage. 50A makes that bare-passage state faithfully visible; 50B permits changed-basis preparation without dropping it; 50C carries that basis through the existing content-addressed 33B transition relationship; 50D roots the first post-change revision in root-v1 and rejoins ordinary edge-v1 through unchanged 34B. Do not infer generic 24C root input, direct root sequence-start support, browser highlighting, automatic adoption, tags, search, or export from these crossings.
